...
public class HoursYearTreeTable extends TreeViewer {
	...
	public HoursYearTreeTable(Tree table) {
		...
		new ColumnValueComparator<HoursYearTableEntry>(this, col, (e1,e2) -> e1.name.compareTo(e2.getName()) );
				
		col = addColumn("Total", 100, SWT.RIGHT);
		new ColumnValueComparator<HoursYearTableEntry>(this, col, (e1,e2) -> Float.compare(e1.getYearSum(), e2.getYearSum()));
		
		for (int i = 0; i < Months.NAMES.length; i++) {
			col = addColumn(Months.NAMES[i], 90, SWT.RIGHT);
			final int month = i;
			new ColumnValueComparator<HoursYearTableEntry>(this, col, (e1,e2) -> Double.compare(e1.getValue(month), e2.getValue(month)));
		}
